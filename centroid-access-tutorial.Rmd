...
# Read in zip code data {#read-zips}
...
# Calculate centroids of zip code boundaries {#calculate-centroids}
...
Then geocode the addresses. This can now be transformed into an `sf` object and be used in the [rest of analysis](#read-resources).
...
The `zctas()` function will pull the last published year of zip code boundaries, which defaults to 2018. Additional functions to pull states, counties, tracts, blocks, and more can be found in the [`tigris` documentation](https://github.com/walkerke/tigris). You can change the year with the `year` argument. **Note: this will take a while to run.**

```{r eval=FALSE}
il_zips <- zctas(state = "IL", year = 2017)
```

Once you pull the boundaries, you can [plot your data](#read-zips) and [calculate the centroids](#calculate-centroids). 