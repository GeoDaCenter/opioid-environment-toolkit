...
* Download census boundaries thorugh the Census API
* Wrangle 
...
Our output will be a .csv file and shapefile (.shp suite) with data at the census tract level.
...
```{r load,message=FALSE, results='hide', eval = FALSE}
library(sf)
library(tidycensus)
library(tidyverse)
library(tigris)
...
```{r download state level, message = FALSE, eval = FALSE}
...
```{r download county level, message = FALSE, eval = FALSE}
...
```{r download tract level, message = FALSE, results='hide', eval = FALSE}
...
Now we can finally save the census tract results with geometry in a shapefile using `write_sf`. 
```{r eval=FALSE}
write_sf(countyShp, "data/IL_County_18.shp")
```
...
--> @Moksha: Also save ZCTA data for Illinois so that we can merge with master shapefile for next tutorial??

--> make sure you save to data/ folder 
...
```{r check variables,  message = FALSE, eval = FALSE}
...
```{r explore variables 1 ,  message = FALSE, eval = FALSE}
...
```{r explore variables 2,  message = FALSE, eval = FALSE}
...
```{r explore variables 3, eval = FALSE}
